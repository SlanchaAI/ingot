...
if you can. For a shared or company-wide deployment, **Sign in with Google** (`AUTH_MODE=oidc`)
adds domain-restricted login and the viewer/proposer/approver/admin roles, with the signed-in email
as the audit actor; see [docs/sso.md](docs/sso.md). For authenticating the MCP serving endpoints
themselves, put an authenticating reverse proxy in front.
